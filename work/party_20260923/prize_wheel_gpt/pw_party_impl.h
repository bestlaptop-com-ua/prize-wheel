...
static volatile uint32_t pwFxAlarmUntilMs = 0;   /* hand-stop strobe end       */
#define PW_HAND_ALARM_MS 5000
#if PW_FX_LED_ENABLE
...
/* 2026-09-23 hand-stop alarm (owner decision): bright red strobe.  3 flashes
 * per second - kept at the 3 Hz photosensitivity guideline, since saturated
 * red flashing faster than that is what can trigger seizures in guests.      */
static void pwLedRenderAlarm(uint32_t nowMs) {
  const bool on = (nowMs % 333u) < 150u;
  fill_solid(pwLeds, PW_NUM_LEDS, on ? CRGB(255, 0, 0) : CRGB::Black);
}
...
static void pwFxLedTask(void*) {
  ...
  for (;;) {
    ...
    uint32_t alarmUntil = pwFxAlarmUntilMs;
    bool alarming = alarmUntil != 0 && (int32_t)(alarmUntil - nowMs) > 0;
    uint8_t mode = pwFxMode;
    if (!pwLedEnabled) {
      fill_solid(pwLeds, PW_NUM_LEDS, CRGB::Black);
    } else if (alarming) {
      pwLedRenderAlarm(nowMs);
    } else if (celebrating) {
...
static void pwFxService(uint32_t nowMs) {
  ...
  /* ---- hand-stop alarm (2026-09-23 owner decision) ----------------------- */
  /* Raised by the control core (handStopAlarm) when a guest stopped the wheel
   * by hand for sure: alarm sound + bright red strobe, instead of the guest
   * jingle and instead of any celebration.                                   */
  const bool handAlarm = handAlarmPending;
  if (handAlarm) {
    handAlarmPending = false;
    pwDfpFlush();
    pwRatchetOn = false;
    pwFanfareAtMs = 0;
    pwFxCelebrateAtMs = 0;
#if PW_FX_AUDIO_ENABLE
    if (pwAudioEnabled) pwI2sAlarm(PW_HAND_ALARM_MS);
#endif
    pwFxAlarmUntilMs = nowMs + PW_HAND_ALARM_MS;
  }

  /* ---- spin-close observer: landing celebration / guest jingle ---------- */
  bool open = spinOpen;
  if (pwPrevSpinOpen && !open && !handAlarm) {
  ...
  /* P8 (2026-09-23): never silently unarmed.  At rest, the fault look (dim
   * steady amber) also shows whenever the NEXT spin could not be steered
   * (takeover off, driver not answering, direction calibration missing,
   * encoder stale...).  Audio is unchanged.                                  */
  bool unarmed = (state == ST_IDLE_STOPPED || state == ST_SOFT_HOLD) && speed <= 0.05f &&
                 !steeringArmed();
  if (faulted || unarmed) {
    pwFxMode = PWL_FAULT;
  } else if (speed > 0.05f ||
...
static void pwS2Service(uint32_t nowMs) {
   ...
   * with test_connection still passing" failure from the bench.
   * 2026-09-23: re-apply and verify at once instead of latching a fault that
   * needed an r; if the driver still disagrees, steering waits (tmcOk=false)
   * and serviceDriverReady() keeps retrying at rest.                         */
  Serial.printf("# S2: TMC config verify FAILED (microsteps=%u,%u expected %u); re-applying\n",
                got, got2, (unsigned)MICROSTEPS);
  if (!pwS2ReconfigVerify()) tmcOk = false;
}
#endif /* PW_S2_ENABLE */
