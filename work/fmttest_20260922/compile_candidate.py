"""Compile an isolated source copy on MILL-PC. Never uploads or opens serial."""
...
args = [str(CLI), 'compile', '--fqbn', FQBN, '--warnings', 'all',
        '--build-path', str(ROOT / 'build'), '--output-dir', str(ROOT / 'firmware'),
        str(ROOT / 'fmttest')]
...
try:
    ...
    if result.returncode == 0:
        hashes = {p.name: hashlib.sha256(p.read_bytes()).hexdigest()
                  for p in (ROOT / 'firmware').glob('*.bin')}
