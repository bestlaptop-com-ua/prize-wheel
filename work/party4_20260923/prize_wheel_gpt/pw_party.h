...
#define PW_S1_ENABLE 0   /* 2026-09-23 OFF: latched faults blocked steering until r (38 unsteered spins on 9/22, FAILURE_MODES A1); faults now auto-clear and are not persisted */
...
class PwMirrorSerial : public Print {
 public:
  ...
  /* 2026-09-23 (FAILURE_MODES F3): UART0 had no TX buffer, so every print
   * blocked the control loop ~87 us per byte beyond the 128-byte FIFO.
   * Must be called before begin().                                          */
  size_t setTxBufferSize(size_t n) { return ::Serial.setTxBufferSize(n); }
