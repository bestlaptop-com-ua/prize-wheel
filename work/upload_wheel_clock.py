...
try:
 assert (root/'wheel_clock_build.exit').read_text().strip()=='0'
 with (root/'wheel_clock_upload.log').open('w',encoding='utf-8') as out:
  r=subprocess.run([cli,'upload','--fqbn',fqbn,'--port','COM7','--input-dir',str(root/'wheel-clock-firmware'),str(root.parent/'prize_wheel_gpt')],stdout=out,stderr=subprocess.STDOUT,timeout=90)
 (root/'wheel_clock_upload.exit').write_text(str(r.returncode))
 if r.returncode==0:
  p=serial.Serial();p.port='COM7';p.baudrate=115200;p.timeout=.1;p.dtr=False;p.rts=False
  try:
   p.open()
   with (root/'wheel_clock_boot.log').open('w',encoding='utf-8',buffering=1) as f:
...
except Exception:(root/'wheel_clock_upload.error').write_text(traceback.format_exc())
