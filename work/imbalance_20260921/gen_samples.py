...
OUT = pathlib.Path(r'C:\Users\Mill\Desktop\prize-wheel\work\imbalance_20260921\\prize_wheel_gpt\\pw_samples.h')
names = {1:'tick',2:'ratchet',3:'drumroll',4:'fanfare',5:'ambience',6:'guest'}
SR = 22050
lines = ['// auto-generated: 16-bit mono PCM @ %d Hz from media/mp3' % SR, '#pragma once', '#include <stdint.h>', '#define PW_SAMPLE_RATE %d' % SR]
table = []
total = 0
...
lines.append('struct PwSample { const char* name; const int16_t* data; uint32_t n; };')
lines.append('static const PwSample PW_SAMPLES[] = {')
for name, n in table:
    lines.append(f'  {{"{name}", pcm_{name}, {n}}},')
lines.append('};')
lines.append('#define PW_NUM_SAMPLES %d' % len(table))
OUT.write_text('\n'.join(lines))
