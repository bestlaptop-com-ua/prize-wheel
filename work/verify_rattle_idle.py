...
try:
 ...
 boot=read_for(4)
 assert 'v2-rattle-diag-20260918' in boot,'wrong build'
 ...
 (root/'rattle_idle_test.json').write_text(json.dumps(report,indent=2))
 print(json.dumps(report,indent=2));print(final)
finally:
 (root/'rattle_idle_test.log').write_bytes(all_data)
