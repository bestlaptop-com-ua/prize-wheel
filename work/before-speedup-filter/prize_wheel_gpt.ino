...
const uint16_t CUR_HOLD1_MA     = 1650; // NEMA23: continuous hold at existing brake current
...
bool controlSafetyChecks(uint32_t nowMs) {
  ...
  if (forward > 0.0f) {
    if (forward < lowestForwardRevS) {
      ...
      // Keep the fault debounce, but do not add repeated 5% command
      // cuts outside the planned ramp, which could worsen tracking loss.
...
void help() {
  Serial.println(F(
    ...
    "# build: v2-smooth-ramp-20260918; planned pulse ramp; 1650mA persistent hold; 16K trace\n"
  ...
  switch (state) {
    ...
    case ST_LANDING_SETTLE: {
      ...
      if (fabsf(degreesForCounts(encoderCountsMT - settleEntryCounts)) >
        ...
        Serial.printf("SPIN#%lu SETTLE-TRAVEL: excess motion during settle; source unknown\n",
                      (unsigned long)spin.number);
        driverFreewheel();
        closeSpin(RES_GUEST_STOPPED);
        candidateStartCounts = encoderCountsMT;
