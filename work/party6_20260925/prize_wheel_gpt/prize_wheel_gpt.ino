 ...
 *
 * 2026-09-23 FAIL-OPERATIONAL (FAILURE_MODES_2026-09-23.md, stage 1a):
 *   - Once the motor has the wheel it never lets go while the wheel moves.
 *     Anomalies in powered states are logged (ANOMALY lines, spin anomalies=)
 *     and handled at rest; they never switch the driver off.  The only
 *     release is a guest re-spin, which starts a new spin that is captured
 *     again.  A wheel that stops following the field (fight/reversal) gets its
 *     pulse train frozen and is held, not released.
 *   - No fault is persisted across power cycles (S1 off) and every fault
 *     except a failed direction probe or a missing STEP timer clears itself
 *     once the wheel rests with healthy feedback.
 *   - No per-rest capture budget: a capture that never energized is retried.
 *   - Flash writes happen only at rest; Serial has a TX buffer; the driver is
 *     waited for at boot instead of latching a fault; the RAM dump is chunked.
 *   - The LEDs show dim amber whenever steering is not armed.
 *
 * State machine:
 ...
 *     forceStop() is used only to freeze the field of a wheel that stopped
 *     following it (energized, 2026-09-23) and to clear a dead queue with the
 *     outputs off; it is always guarded by isRampGeneratorActive() (AUDIT s1).
...
// 2026-09-23 flash 3: 2200 -> 2700 mA (+23% pull-out torque).  Slips of 1-3 electrical
// cycles (7.2 deg each) in 8-18% of the 9/23 spins; the 1 kHz trace of flash-2 spin 1 ran
// at ~1.8 deg load angle (the torque peak) in the final approach.  Motor rated 3 A;
// TMC5160 at R_SENSE 0.075 tops out at 3.06 A RMS.  Braking lasts ~5 s per spin.
...
struct DiagnosticSample {      // 28 bytes; 3072 samples = ~86 KB, ~3 s at 1 kHz
  ...
  uint16_t curMa10;            // commanded motor current / 10 mA (flash 3: 16 bit, 2700 mA > 2550)
};
...
struct SpinRecord {
  ...
  float maxSyncErrDeg;         // 2026-09-23: max |encoder - STEP| travel since torque-on
...
// 2026-09-23 fail-operational (stage 1a).
// Capture retries: an attempt that never energized had no mechanical effect,
// so the next one may start after this short back-off (queue drain + fresh
// velocity), for as long as the wheel moves.
const uint16_t CAPTURE_RETRY_MS = 60;
uint32_t captureRetryAtMs = 0;
uint32_t lastAbandonLogMs = 0;
uint16_t abandonsSinceLog = 0;
// Anomalies seen while the motor steers the wheel (logged, never released).
uint8_t spinAnomalies = 0;
const uint32_t ANOMALY_REPEAT_MS = 5000;  // same-code log suppression window
FaultCode anomalyLastCode = FC_NONE;
uint32_t anomalyLastLogMs = 0;
// The wheel stopped following the field (fight/reversal): the pulse train is
// frozen and the energized field holds the wheel where it is.
bool fieldFrozen = false;
// Open-loop fallback: remaining distance from the STEP count since the last
// fresh encoder tick, used only while the encoder is stale or implausible.
int32_t stepRefPos = 0;
float stepRefRemainDeg = 0.0f;
bool stepRefValid = false;
int32_t syncRefPos = 0;          // step/encoder origin at torque-on (telemetry)
int32_t syncRefCounts = 0;
// 2026-09-23 flash 3: slip telemetry.  One line per further electrical cycle
// (7.2 deg) of encoder-vs-STEP drift; the no-slip baseline is 2-7 deg (pickup
// snap up to 3.6 deg plus encoder nonlinearity), so the first line is at 8 deg.
const float SLIP_LOG_FIRST_DEG = 8.0f;
const float SLIP_CYCLE_DEG = 7.2f;
uint8_t slipLogged = 0;
bool syncRefValid = false;       // the STEP origin above still belongs to this capture
// Dare recovery keeps trying at rest (P7); attempts alternate direction.
const uint8_t DARE_RECOVERY_MAX_ATTEMPTS = 4;
// Friction fits persist only at rest (a flash write stalls the loop ~20-30 ms).
bool frictionSavePendingCw = false, frictionSavePendingCcw = false;

// Hand-stop alarm (2026-09-23, owner: "If stops by hand and we are sure it is,
// then play alarm sound and bright red strobe").  Only a deceleration that no
// motor or friction can produce counts: full motor torque (~1.9 Nm on the
// 0.94 kgm2 wheel) plus friction slows the wheel by at most ~0.4 rev/s2.
// Evidence, on valid encoder samples 25 ms apart (3-sample medians, so a one-
// tick encoder spike cannot define the peak): within 400 ms the speed fell from
// >= 0.12 rev/s to <= 40 % of that peak at >= 0.8 rev/s2 averaged over >= 100 ms,
// and the wheel then comes to rest within 1.5 s without being pushed again.
// A hand stop: alarm + red strobe, and NO dare recovery (the wheel stays where
// the hand left it).
const float HAND_MIN_SPEED_REV_S = 0.12f;
const float HAND_DROP_FRACTION = 0.40f;
const float HAND_DECEL_REV_S2 = 0.80f;
const uint16_t HAND_MIN_SPAN_MS = 100;
const uint16_t HAND_WINDOW_MS = 400;
const uint16_t HAND_REST_WITHIN_MS = 1500;
const uint8_t HAND_RING = 24;
struct HandSample { uint32_t ms; float v; };
HandSample handRing[HAND_RING];
uint8_t handHead = 0, handCount = 0;
uint32_t handLastSampleMs = 0;
bool handSuspect = false;
uint32_t handSuspectMs = 0;
float handSuspectVmax = 0.0f;
volatile bool handAlarmPending = false;   // consumed by the FX observer (pw_party_impl.h)

// Gentle carry (2026-09-23, owner: "Gently carry"): chooser pass 5, quality 4.
const float CARRY_TRIGGER_DEG = 5.0f;     // natural stop inside a dare or this close (shadow capture needs 5)
const float CARRY_CLEAR_DEG = 10.0f;      // carry target clearance from any dare edge
const uint8_t Q_CARRY = 4;

// Re-push while braking (2026-09-23, stage-1a review): a wheel clearly faster
// than the field for this long is being pushed (or has slipped ahead): release
// it as a new spin, which is captured again, instead of grinding against it.
const float REPUSH_MARGIN_REV_S = 0.15f;
const uint16_t REPUSH_CONFIRM_MS = 150;
// flash 3: a push during LANDING_SETTLE (the old SETTLE-DRAG release used to catch
// it) is a new spin once the wheel runs above this speed continuously for
// REPUSH_CONFIRM_MS.  No dip tolerance here: a rotor ratcheting through the static
// field reads as spiky speed on the motor-shaft encoder, and it must stay held.
const float SETTLE_RESPIN_REV_S = 0.30f;
uint32_t repushSinceMs = 0;
uint32_t repushLastTrueMs = 0;
...
void recordDiagnostic(uint32_t dtGoodUs, int16_t delta, uint8_t flags) {
  ...
  s.curMa10 = (uint16_t)(g_currentMa / 10);
...
void driverFreewheel() {
  ...
  if (stepper) {
    // 2026-09-23: guard on the RAMP GENERATOR, not isRunning(): isRunning() stays true while
    // the last queued steps drain after the generator went idle, and forceStop() in that window
    // leaves the one-shot stop flag armed for the next capture (AUDIT s1; review of stage 1a).
    if (stepper->isRampGeneratorActive()) stepper->forceStop();
    stepper->setJumpStart(0);
  }
  cmdRevS = 0.0f;   // dead control episode must not linger in telemetry
}

// Driver link check (SPI on this board; the TMC_UART names are legacy).
// Called only with the wheel at rest.  Updates tmcOk only: since 2026-09-23 a
// driver that does not answer (24 V still off at boot) is not a latched fault;
// steering simply waits and serviceDriverReady() re-checks at rest (F2/P9).
bool checkTmcUartQuiet() {
  tmcOk = (driver.test_connection() == 0);
  return tmcOk;
}
...
bool checkTmcUartRaw() {
  ...
  tmcOk = (result == 0);
  if (!tmcOk) {
    Serial.printf("# TMC SPI check FAIL code=%u: steering waits until the driver answers (24 V?)\n", result);
  }
  return tmcOk;
}

// Re-assert the boot configuration, then the registers of the CURRENT stage
// (driverConfig() alone leaves 1450 mA and freewheel off).  Used at rest and,
// fail-operational (P1), while powered after the driver reported a reset: its
// outputs are already dead until TOFF is rewritten, so this can only help.
void driverReapplyForStage() {
  driverConfig();
  if (currentStage == CS_FREEWHEEL) {
    driver.freewheel(1);
    driver.ihold(0);
  } else {
    driver.freewheel(0);
    if (g_currentMa > 0) driver.rms_current(g_currentMa, 1.0);
  }
}
...
void finishFrictionCapture(const char* reason, PwFitEnd end) {
  ...
  if (fitDir > 0) {
    ...
    ++cwFitCount;
    // 2026-09-23 (F3): this runs while the wheel is about to be captured; the
    // flash write stalled the loop past the encoder gap limit and cost spins
    // 5 and 13 their capture.  Persist later, at rest (serviceDeferredSaves).
    if (cwFitCount >= FIT_PERSIST_MIN_FITS) frictionSavePendingCw = true;
    ...
    ++ccwFitCount;
    if (ccwFitCount >= FIT_PERSIST_MIN_FITS) frictionSavePendingCcw = true;  // at rest (F3)
    Serial.printf("SPIN#%lu FRICTION dir=-1 fitC=%.4f fitB=%.4f pairs=%d -> c=%.4f b=%.4f fits=%u\n",
...
TargetChoice chooseSafeTarget(int dir, float curAngle, float speedRevS) {
  ...
  // Pass 5 - gentle carry (2026-09-23, owner decision), tried BEFORE the hard
  // braking passes 3/4: on 9/23 both pass-3 stops (planned at 2-3x the natural
  // deceleration) slipped the rotor by ~120 deg.  Passes 1-2 found nothing and
  // the natural stop is dare territory: instead of braking hard to a nearer
  // wedge, or letting the wheel die on the dare and nudging it off afterwards,
  // capture it and carry it at about its natural deceleration to the first safe
  // interior beyond the natural stop (>= CARRY_CLEAR_DEG from any dare edge).
  // The only pass that aims past the natural stop: the motor adds a little
  // energy against friction.
  if (!out.found && naturalDeg > 0.0f) {
    float natAng = fmodf(curAngle + (float)dir * naturalDeg, 360.0f);
    if (natAng < 0.0f) natAng += 360.0f;
...
// 2026-09-23: a rejected call is reported to the caller, which picks the
// fail-operational response (abandon an unpowered capture, ramp-stop a powered
// one, fail a probe).  It no longer latches a fault that would release the
// wheel from inside a powered state.
bool fasSetSpeedHz(uint32_t hz) {
  if (!stepper || stepper->setSpeedInHz(hz) != 0) {
    Serial.printf("# FAS setSpeedInHz(%lu) rejected\n", (unsigned long)hz);
    return false;
  }
  return true;
}
...
bool fasSetAcceleration(uint32_t sps2) {
  if (!stepper || stepper->setAcceleration((int32_t)sps2) != 0) {
    Serial.printf("# FAS setAcceleration(%lu) rejected\n", (unsigned long)sps2);
    return false;
  }
  return true;
}
...
bool fasRun(int fasSign) {
  ...
  if (rc != MoveResultCode::OK) {
    Serial.printf("# FAS %s rejected\n", (fasSign > 0) ? "runForward" : "runBackward");
    return false;
  }
  return true;
}
...
void startSpinEvent(int confirmedDir, uint32_t nowMs) {
  // 2026-09-23 (P3): every spin, including a guest re-spin and a spin right
  // after boot or a fault clear, may be captured; there is no attempt budget.
  captureCycle.newSpin();
  captureRetryAtMs = nowMs;
  lastAbandonLogMs = 0;
  abandonsSinceLog = 0;
  spinAnomalies = 0;
  anomalyLastCode = FC_NONE;   // a new spin logs its anomalies afresh
  fieldFrozen = false;
  handReset();                 // a new (or re-)spin is never a hand stop
  repushSinceMs = 0;
  ++spinCounter;
...
void closeSpin(SpinResult result) {
  if (!spinOpen) return;
  spinOpen = false;
  syncRefValid = false;  // flash 3: no sync tracking once the spin is closed
  spin.result = result;
  if (faultCode != FC_NONE) spin.fault = faultCode;  // else keep the first anomaly
  ...
  Serial.printf(
      ...
      (unsigned)captureCycle.attempts(), (unsigned)spinAnomalies, spin.maxSyncErrDeg,
...
// 2026-09-23 fail-operational (FAILURE_MODES_2026-09-23.md, P1).
// True while the energized motor steers or holds the wheel: capture, braking,
// settling, the landing hold, or a dare-recovery move.  Nothing in these
// states may switch the driver off because of a monitor or a fault.
bool motorSteering() {
  return state == ST_SPEED_MATCH_CAPTURE || state == ST_CONTROLLED_DECEL ||
         state == ST_LANDING_SETTLE || state == ST_SOFT_HOLD ||
         state == ST_RECOVERY_NUDGE;
}

// An anomaly while the motor steers the wheel: logged and recorded in the spin
// summary, never acted on by releasing the wheel.  The first anomaly of a spin
// is reported as its fault= field.  A condition that persists (e.g. a dead SPI
// link during a long hold) logs once per ANOMALY_REPEAT_MS; every new spin
// logs it again (startSpinEvent resets the episode).
void controlAnomaly(FaultCode code, const char* detail) {
  const uint32_t nowMs = millis();
  if (spinOpen && spin.fault == FC_NONE) spin.fault = code;
  if (code == anomalyLastCode && nowMs - anomalyLastLogMs < ANOMALY_REPEAT_MS) return;
  anomalyLastCode = code;
  anomalyLastLogMs = nowMs;
  if (spinAnomalies < 255) ++spinAnomalies;
  Serial.printf("ANOMALY code=%s detail=%s state=%s angle=%.1f wedge=%d omega=%.3f cmd=%.3f "
                "(motor keeps control)\n",
                faultName(code), detail, stateName(state), wheelAngleDeg(),
                currentWedge(), omega, cmdRevS);
}

// Hard stop of the controller: switches the driver off and latches a fault
// (unarmed, LEDs amber).  Since 2026-09-23 it is reached only with the motor
// NOT steering the wheel (see enterFault) or, deliberately, from a hold at rest
// whose encoder is gone.  Nothing is persisted (S1 off); every code except a
// failed direction probe or a firmware/STEP-timer fault auto-clears at rest.
void latchFault(FaultCode code, const char* detail) {
  ...
  faultCode = code;
  PW_S1_PERSIST(code);  // compiled out: PW_S1_ENABLE 0 since 2026-09-23
...
// The single fault entry used by the control code.  While the motor steers the
// wheel this NEVER releases it: the fault becomes an anomaly and control goes
// on (a belt-and-braces guard for any path not converted explicitly).
void enterFault(FaultCode code, const char* detail) {
  if (motorSteering()) {
    controlAnomaly(code, detail);
    return;
  }
  latchFault(code, detail);
}
...
void startDirectionProbe() {
  ...
  if (!fasSetSpeedHz(DIR_PROBE_SPEED_HZ) || !fasSetAcceleration(DIR_PROBE_ACCEL_SPS2)) {
    probeFail("FAS speed/acceleration rejected");  // releases the at-rest probe
    return;
  }
...
bool startDareRecovery(const char* why) {
  ...
  int dir, wt;
  if (!isDare(wedge)) {
    // 2026-09-23: resting in a SAFE wedge within DARE_PROXIMITY of a dare edge:
    // step to the centre of this same wedge (a few degrees, away from the
    // dare) instead of a whole wedge over.
    wt = wedge;
    float centerHere = wedge * WEDGE_DEG + 0.5f * WEDGE_DEG;
    dir = (forwardDistanceDeg(1, angle, centerHere) <= 180.0f) ? 1 : -1;
  } else {
    float g = wheelGravity.forwardAccel(angle * DEG_TO_RAD, 1);
    dir = (fabsf(g) < 0.08f) ? (spinDir != 0 ? spinDir : 1) : (g > 0.0f ? 1 : -1);
    // 2026-09-23 (P7): recovery retries until it succeeds; every second
    // attempt goes the other way in case one side cannot be reached.
    if (dareRecoveryAttempts % 2 == 1) dir = -dir;
    wt = wedge + dir;
  ...
  syncRefValid = false;  // flash 3: the capture's STEP origin is gone after this re-zero
  stepper->setCurrentPosition(0);
  stepper->setJumpStart(0);
...
// P8 (2026-09-23): everything the NEXT spin needs to be steered.  When false at
// rest the LEDs show dim amber, so the wheel is never silently unarmed.
bool steeringArmed() {
  return faultCode == FC_NONE && takeoverEnabled && motorDirectionCalibrated &&
         motorPositiveEncoderSign != 0 && tmcOk && stepper && captureTimerReady &&
         encoderPositionFresh();
}
...
bool tryReserveTarget(uint32_t nowMs) {
  ...
  if ((int32_t)(nowMs - captureRetryAtMs) < 0) return false;  // retry back-off (P3)
  // A hand is stopping the wheel: don't engage the motor onto it (flash 2).
  if (handSuspect && nowMs - handSuspectMs < HAND_REST_WITHIN_MS) return false;
  ...
  // Reject short runway before reserving.  (Since 2026-09-23 a pickup that
  // fails before torque-on is retried; nothing was energized.)
  if (choice.runwayDeg < MIN_RESERVE_RUNWAY_DEG) return false;

  // Invariant: a dare can never be reserved.  Refuse it and try again next
  // tick; latching a fault here would leave the spin unsteered.
  if (isDare(choice.wedge)) {
    Serial.printf("# TARGET_INVARIANT: chooser returned dare wedge %d; refused\n", choice.wedge);
    captureRetryAtMs = nowMs + CAPTURE_RETRY_MS;
    return false;
  }
...
// A capture that fails before torque-on (EN never went low) has had no
// mechanical effect.  2026-09-23 (P3): return to SPIN_RELEASED and retry after
// CAPTURE_RETRY_MS for as long as the wheel moves; the next reservation may
// pick a different safe wedge (invisible, nothing was energized).
// Logging is rate-limited: the first abandon of a spin, then one line per
// 500 ms with the count, so a retry storm cannot stall the loop on Serial.
bool abandonLogDue(uint32_t nowMs) {
  return lastAbandonLogMs == 0 || nowMs - lastAbandonLogMs >= 500;
}
...
void abandonCapture(const char* reason) {
  ...
  ++abandonsSinceLog;
  if (abandonLogDue(nowMs)) {
    Serial.printf("# CAPTURE_ABANDON reason=%s attempts=%u abandons=%u; retry in %u ms\n",
        reason, (unsigned)captureCycle.attempts(), (unsigned)abandonsSinceLog,
        (unsigned)CAPTURE_RETRY_MS);
    lastAbandonLogMs = nowMs;
    abandonsSinceLog = 0;
  }
}

static uint32_t drvHealthRetries = 0;
static uint32_t drvReapplyCount = 0;
// Driver health: status, flags, version and microstep readback.
// poweredRepair (2026-09-23, P1): also repair a driver that reset while the
// motor is energized - after a supply dip its outputs are dead (TOFF=0) until
// the configuration is rewritten, so rewriting it is the only way back to
// torque.  Without it, the repair runs only with the outputs off.
bool driverHealthy(bool poweredRepair) {
  ...
  // Re-apply the config and re-read instead of abandoning every capture until
  // someone types r.  2026-09-23: also with uv_cp set alongside (GSTAT=05 was
  // seen on 9/21) and, when poweredRepair, with the outputs enabled.
  const bool enHigh = digitalRead(PIN_EN) == HIGH;
  if (gst != 0xFF && (gst & 0x01) && driver.version() == 0x30 && (enHigh || poweredRepair)) {
    driverReapplyForStage();
    ++drvReapplyCount;
    Serial.printf("# driver reset flag seen (GSTAT=%02X EN=%s): config re-applied (#%lu)\n",
                  (unsigned)gst, enHigh ? "off" : "ON", (unsigned long)drvReapplyCount);
...
bool captureDriverHealthy() { return driverHealthy(false); }
...
bool prepareCapturePlan(uint32_t hz, float forward) {
  ...
  // A gentle carry (quality 4) aims past the natural stop by design, bounded here.
  const bool carry = spin.targetQuality == Q_CARRY;
  if (!isfinite(naturalRemaining) ||
      (!carry && remaining > naturalRemaining * 1.10f + 5.0f) ||   // shadow targets sit at natural-6: a 2.5% speed drop while arming moves natural by 5%
      (carry && remaining > naturalRemaining + 3.0f * WEDGE_DEG)) return false;
...
void serviceCaptureArming(uint32_t nowMs) {
  (void)nowMs;
  if (!controlAvailable() || !stepper || !captureCycle.arming() ||
  ...
  if (decision == PwCaptureArm::REJECT) {
    const bool logIt = abandonLogDue(millis());
  ...
  captureArm.verified = false;
  currentStage = CS_CAPTURE; // registers already contain CUR_CAPTURE_MA; no traffic at EN edge
  ...
  fieldFrozen = false;
  repushSinceMs = 0;
  // 2026-09-23: STEP-count reference for the open-loop fallback and the
  // encoder-vs-STEP sync telemetry (spin summary syncErr=).
  stepRefPos = syncRefPos = stepper->getCurrentPosition();
  stepRefRemainDeg = remainingTargetDeg();
  stepRefValid = true;
  syncRefCounts = encoderCountsMT;
  syncRefValid = true;
  slipLogged = 0;
...
// Powered driver-health cadence (100 ms).  2026-09-23 (P1): a failed check is
// repaired where possible (driver reset -> configuration re-applied, see
// driverHealthy) and logged as an anomaly; it never switches the driver off.
// STEP/DIR keep driving the motor whatever the SPI link does.  The legacy
// TMC_UART label also covers this board's SPI driver/status checks.
void controlDriverSafe(uint32_t nowMs) {
  static uint32_t unhealthySinceMs = 0;
  const uint32_t sinceLastCheck = nowMs - lastPoweredHealthMs;
  if (sinceLastCheck < 100) return;
  if (sinceLastCheck > 500) unhealthySinceMs = 0;  // not a continuous episode
  lastPoweredHealthMs = nowMs;
  const uint32_t reapplyBefore = drvReapplyCount;
  const bool healthy = driverHealthy(true);
  if (drvReapplyCount != reapplyBefore && state == ST_SOFT_HOLD) {
    // Stage-1a review: the re-applied field comes back at a new electrical
    // phase, so the rotor may have snapped up to 3.6 deg.  Never read that snap
    // as a guest push; re-judge the landing instead (hold, or dare recovery).
    ...
    stateEnteredMs = nowMs;
    Serial.println(F("# hold: driver config re-applied; re-judging the landing"));
  }
  if (healthy) {
    unhealthySinceMs = 0;
    if (!tmcOk) {
      tmcOk = true;
      Serial.println(F("# driver healthy again; steering available"));
    }
    return;
  }
  controlAnomaly(FC_TMC_UART, "powered SPI driver health/config/status failed");
  // P8: a driver that stays unhealthy (e.g. 24 V lost during the hold) means the
  // NEXT spin cannot be steered: say so (LEDs amber) without releasing anything.
  if (unhealthySinceMs == 0) unhealthySinceMs = nowMs;
  else if (tmcOk && nowMs - unhealthySinceMs >= 1000) {
    tmcOk = false;
    Serial.println(F("# driver unhealthy for 1 s while powered: next spins wait for it"));
  }
}

// The wheel stopped following the field (fight or reversal: a hand, or a lost
// rotor).  Further pulses would only grind past it and letting go would leave
// it wherever it coasts.  Freeze the pulse train where it is and keep the coils
// at braking current: the field holds the wheel, and the normal settle ->
// verdict -> (dare recovery) path takes over once the queue drains (~20 ms).
void freezeField(FaultCode why, const char* detail) {
  if (fieldFrozen) return;
  fieldFrozen = true;
  spin.frozen = 1;
  controlAnomaly(why, detail);
  stopRequested = true;
  if (stepper && stepper->isRampGeneratorActive()) stepper->forceStop();  // guarded (AUDIT s1)
  cmdRevS = 0.0f;
}

// Telemetry for stage 1c: how far the wheel (encoder) and the field (STEP
// count) have drifted apart since torque-on.  One full step is 1.8 deg; the
// pickup snap alone can be up to 3.6 deg.
void trackSyncError() {
  if (!stepper || !syncRefValid) return;
  const float encDeg = degreesForCounts(takeoverDir * (encoderCountsMT - syncRefCounts));
  const float stepDeg = (float)(stepper->getCurrentPosition() - syncRefPos) * 360.0f /
                        WHEEL_USTEPS_PER_REV;
  const float signedErr = encDeg - stepDeg;   // + : wheel ahead of the field
  const float err = fabsf(signedErr);
  if (err > spin.maxSyncErrDeg) spin.maxSyncErrDeg = err;
  // 2026-09-23 flash 3: say when and where each slip happens (telemetry only).
  if (slipLogged < 12 && err >= SLIP_LOG_FIRST_DEG + SLIP_CYCLE_DEG * (float)slipLogged) {
    ++slipLogged;
    const char* phase = (state == ST_SPEED_MATCH_CAPTURE) ? "pickup"
                      : (state == ST_LANDING_SETTLE)      ? "settle"
                      : stopRequested                     ? "ramp" : "decel";
    Serial.printf("SPIN#%lu SLIP %u: wheel %s field by %.1f deg, %lu ms after torque-on, "
                  "wheel %.3f field %.3f rev/s, angle %.1f, remain %.1f, %s\n",
                  (unsigned long)spin.number, (unsigned)slipLogged,
                  signedErr >= 0.0f ? "ahead of" : "behind", err,
                  (unsigned long)(millis() - captureStartMs), omega * (float)takeoverDir,
                  fasWheelRevS(), wheelAngleDeg(), remainingTargetDeg(), phase);
  }
}

// Remaining distance for the braking profile: from the encoder while it is
// fresh and plausible; otherwise (P1, open loop) from the STEP count since the
// last good encoder tick, assuming the rotor stays coupled to the field.
float controlRemainingDeg(bool* openLoop) {
  const float enc = remainingTargetDeg();
  const bool encOk = encoderPositionFresh() && enc <= spin.runwayDeg + 30.0f && enc >= -180.0f;
  if (encOk || !stepper || !stepRefValid) {
    if (encOk && stepper) {
      stepRefPos = stepper->getCurrentPosition();
      stepRefRemainDeg = enc;
      stepRefValid = true;
    }
    *openLoop = false;
    return enc;
  ...
  *openLoop = true;
  const float stepDeg = (float)(stepper->getCurrentPosition() - stepRefPos) * 360.0f /
                        WHEEL_USTEPS_PER_REV;
  return stepRefRemainDeg - stepDeg;
}

// Shared monitors for the powered capture/braking states.  2026-09-23 (P1):
// no monitor switches the driver off.  Returns false only when the state was
// changed (a guest re-spin handed back to spin detection) and the caller must
// stop processing this pass.
bool controlSafetyChecks(uint32_t nowMs) {
  controlDriverSafe(nowMs);
  // 1. Encoder position: while stale, braking continues open-loop from the
  //    STEP count (serviceDecelTick) and the kinematic monitors pause.
  if (!encoderPositionFresh()) {
    controlAnomaly(FC_ENCODER_STALE, "position stale in control: braking open-loop");
    speedupWatch.reset();
    fightSinceMs = oppositeSinceMs = guestOverrideAboveMs = 0;
    return true;
  }
  // 2. Velocity may re-prime for a few tens of ms (normal); longer is logged.
  if (!encoderVelocityValid) {
    ...
    if (velocityLossSinceMs == 0) velocityLossSinceMs = nowMs;
      ...
      controlAnomaly(FC_ENCODER_VELOCITY, "velocity unusable in control");
    return true;  // keep braking; monitors resume on the next valid sample
  }
  ...
  // 3. Guest override: the wheel was grabbed and re-spun mid-control.  That is
  // a NEW spin: the coils float while the hand drives it, and spin detection
  // captures it again (P3; there is no attempt budget).  This is the only
  // release in powered control.  (The instantaneous 0.80 rev/s CONTROL_OVERSPEED
  // fault that used to pre-empt this path is gone: AUDIT_2026-09-22 s6.)
  ...
  // 3b. Re-push while braking (2026-09-23, stage-1a review): a wheel clearly
  // faster than the field for REPUSH_CONFIRM_MS is being pushed (or has slipped
  // ahead of the field).  Grinding against it for the rest of the committed ramp
  // helps nobody: hand it back to spin detection as a new spin, which is
  // captured again with a fresh plan (P3).
  // Debounce tolerates short dips (rotor chatter on the coupling): the episode
  // ends only after 60 ms without the condition.
  const float fieldRevS = fasWheelRevS();
  const bool faster = !fieldFrozen && nowMs - captureStartMs >= PICKUP_COHERENCE_MS &&
                      forward > fieldRevS + REPUSH_MARGIN_REV_S;
  if (faster) {
    if (repushSinceMs == 0) repushSinceMs = nowMs;
    repushLastTrueMs = nowMs;
    if (nowMs - repushSinceMs >= REPUSH_CONFIRM_MS) {
      Serial.printf("SPIN#%lu RE-PUSH mid-control: wheel %.3f vs field %.3f rev/s; released as a new spin\n",
                    (unsigned long)spin.number, forward, fieldRevS);
      driverFreewheel();
      closeSpin(RES_GUEST_RESPUN);
      startSpinEvent(takeoverDir, nowMs);
      state = ST_SPIN_PUSH;
      stateEnteredMs = nowMs;
      return false;
    }
  } else if (repushSinceMs != 0 && nowMs - repushLastTrueMs > 60) {
    repushSinceMs = 0;
  }

  // 4. Sustained reversal (a hand pulling back, or a DIR fault driving the
  // wheel backward): never follow it and never let go - freeze and hold.
  if (forward < -OPPOSITE_ABORT_REV_S) {
    if (oppositeSinceMs == 0) oppositeSinceMs = nowMs;
    else if (nowMs - oppositeSinceMs >= OPPOSITE_ABORT_MS)
      freezeField(FC_UNEXPECTED_REVERSAL, "sustained opposite motion: field frozen, wheel held");
  } else {
    oppositeSinceMs = 0;
  }

  // 5. Speed-up: the rotor hunting on the motor's magnetic spring (the command
  // itself never rises).  The plan stays valid: log it and re-baseline.
  ...
  if (sustainedSpeedup) {
    Serial.printf("# SPEEDUP median=%.3f baseline=%.3f rise=%.3f sustained_ms=%lu (logged; braking continues)\n",
                  speedupWatch.median(), speedupWatch.baseline(), speedupWatch.rise(),
                  (unsigned long)speedupWatch.sustainedMs(nowMs));
    controlAnomaly(FC_SUSTAINED_SPEEDUP, "sustained robust speed rise under power");
    speedupWatch.reset();
  }

  // 6. Fight: the wheel far below the *actual* field speed (a hand holding it,
  // step loss, or a DIR fault) - freeze and hold, as for a reversal.  (At very
  // low command speeds a natural early stall is handled by the profile.)
  float fasActual = fasWheelRevS();
  if (!fieldFrozen && nowMs - captureStartMs >= FIGHT_GRACE_MS &&
    ...
    if (fightSinceMs == 0) fightSinceMs = nowMs;
      ...
      freezeField(FC_MOTOR_FIGHT, "wheel far below field speed: field frozen, wheel held");
  } else {
    fightSinceMs = 0;
  }

  // 7. Plausibility of the remaining distance (an encoder turn error): the
  // profile runs from the STEP count meanwhile (controlRemainingDeg).
  float remaining = remainingTargetDeg();
  if (remaining > spin.runwayDeg + 30.0f || remaining < -180.0f)
    controlAnomaly(FC_POSITION_IMPOSSIBLE, "remaining distance impossible: STEP count used");

  // 8. Time budget: finish the stop instead of releasing.
  if (nowMs - controlStartMs >= TAKEOVER_TIMEOUT_MS && !stopRequested) {
    controlAnomaly(FC_TAKEOVER_TIMEOUT, "controlled phase too long: ramp stop");
    stopRequested = true;
...
void serviceDecelTick(uint32_t nowMs) {
  ...
  // 2026-09-23 (P1): the encoder remaining distance, or - while the encoder is
  // stale or implausible - the STEP-count estimate (open loop).
  bool openLoop = false;
  float remaining = controlRemainingDeg(&openLoop);
  ...
  // (Not during a gentle carry: there the field must keep the wheel rolling.)
  if (!openLoop && spin.targetQuality != Q_CARRY &&
      encoderVelocityValid && forward < newCmd - couplingSlack) {
  ...
  // Invariant: commanded motor speed never increases after capture.  An
  // internal violation is clamped and logged; the stop goes on (P1).
  if (newCmd > prevCmd + 1.0e-4f)
    controlAnomaly(FC_MONOTONIC_VIOLATION, "command tried to increase: clamped");
  ...
  if (applyNow && stepper && !stepper->isStopping()) {
    if (!fasSetSpeedHz(hz)) {
      // P1: the library refused the new speed; finish with the planned ramp.
      controlAnomaly(FC_STEPPER_API, "setSpeedInHz rejected: ramp stop");
      stopRequested = true;
      stepper->stopMove();
      return;
    }
...
void landingVerdict() {
  ...
  // Owner decision 2 (2026-09-23): a guest stopped the wheel by hand - alarm,
  // red strobe, and the wheel stays where the hand left it (no recovery move).
  if (handStopConfirmed(settleStillSinceMs)) {
    handStopAlarm("controlled stop");
    closeSpin(RES_GUEST_STOPPED);
    holdAnchorValid = false; holdDeflectSinceMs = 0;
    if (stepper && stepper->isRampGeneratorActive()) stepper->forceStop();
  ...
  const bool dareZone = isDare(wedge) || dareDistanceDeg(angle) < DARE_PROXIMITY_FAULT_DEG;
  if (dareZone) {
    // 2026-09-23 (P7): never rest on a dare.  Walk the wheel off and keep
    // trying (alternating sides) instead of latching a fault after two tries;
    // the latch left the wheel on the dare AND every following spin unsteered.
    if (dareRecoveryAttempts < DARE_RECOVERY_MAX_ATTEMPTS && startDareRecovery("after control"))
      return;
    Serial.printf("SPIN#%lu DARE_RECOVERY gave up after %u attempts: holding at %.1f wedge %d\n",
                  (unsigned long)spin.number, (unsigned)dareRecoveryAttempts, angle, wedge);
    controlAnomaly(FC_LANDING_UNSAFE, isDare(wedge) ? "at rest on a dare" : "at rest on a dare boundary");
  }
  SpinResult result;
  if (dareZone) {
    result = RES_FAULTED;                 // honest: LANDED-DARE is printed by closeSpin
  } else if (edgeDist < LANDING_INTERIOR_MIN_DEG) {
  ...
  closeSpin(result);
  holdAnchorValid = false; holdDeflectSinceMs = 0;
  if (stepper && stepper->isRampGeneratorActive()) stepper->forceStop();   // guarded: on an idle ramp generator forceStop() leaves the FastAccelStepper one-shot stop flag armed and it cancels the next capture (AUDIT_2026-09-22.md s1)
  setCurrentStage(CS_HOLD1);
  state = ST_SOFT_HOLD;
  stateEnteredMs = millis();
}
...
/*                          DIAGNOSTIC DUMP                                   */
/* ========================================================================== */
// A7 (2026-09-23): the dump used to print ~1700 lines in one go and block the
// loop for ~12 s, so a spin during a dump was neither detected nor steered.
// It now streams from serviceDiagnosticDump(), a few lines per loop pass and
// only while the TX buffer keeps DUMP_TX_RESERVE bytes free for control logs.
const int DUMP_TX_RESERVE = 3072;
bool dumpActive = false;
uint16_t dumpNext = 0, dumpFirst = 0;
...
void dumpDiagnostics() {
  ...
  dumpFirst = (diagnosticHead + DIAG_CAPACITY - diagnosticCount) % DIAG_CAPACITY;
  dumpNext = 0;
  dumpActive = true;
}
...
void serviceDiagnosticDump() {
  ...
  for (uint8_t n = 0; n < 4 && dumpNext < diagnosticCount; ++n) {
    if (Serial.availableForWrite() < DUMP_TX_RESERVE) return;
    const DiagnosticSample& s = diagnosticBuffer[(dumpFirst + dumpNext) % DIAG_CAPACITY];
...
/* ========================================================================== */
void help() {
  Serial.println(F("# build: party6-20260925 (flash 6: a free coast that stops is held at 800 mA) on party5-20260925 (flash 5: weak pushes coast free when their stop is safe, 3.0 A) on party4-20260923 (flash 4: catch speed varies per spin 0.40/0.35/0.30/0.25) on party3-20260923 (flash 3: 2.7 A braking, holds through settle drags, SLIP log) on party-20260923 (flash 2: hand-stop alarm, carry, re-push) on failop-20260923 (stage 1a) PARTY; takeover defaults OFF"));
  Serial.println(F(
...
void handleCommandChar(char command) {
  switch (command) {
    ...
    case 's':
      ...
      break;
    case 'd':
      if (dumpActive) Serial.println(F("# DIAG dump still streaming; d ignored"));
      else if (!diagnosticCapture) startDiagnosticCapture();
...
void setup() {
  ...
  Serial.setTxBufferSize(8192);  // 2026-09-23 (F3): prints no longer block the loop
  Serial.begin(115200);
  delay(300);
  ...
  // F2 (2026-09-23): no latched fault when the driver is not up yet (ESP
  // booted before the 24 V supply); serviceDriverReady() configures it as soon
  // as it answers, and until then the LEDs show the wheel is not armed.
  if (!checkTmcUartRaw()) Serial.println(F("# driver not ready at boot: steering waits for it"));
...
// F2/P9 (2026-09-23): the ESP can boot before the 24 V supply, and a supply
// dip can reset the driver.  Instead of a latched fault, steering waits: at
// rest, re-check the driver once a second and configure + verify it the moment
// it answers.  Logged once a minute while it stays silent.
void serviceDriverReady(uint32_t nowMs) {
  static uint32_t lastTryMs = 0, lastLogMs = 0;
  if (tmcOk || !stepper) return;
  if (state != ST_IDLE_STOPPED && state != ST_FAULT_LATCHED) return;
  if (encoderVelocityValid && fabsf(omega) > STILL_REV_S) return;
  if (nowMs - lastTryMs < 1000) return;
  lastTryMs = nowMs;
  if (checkTmcUartQuiet() && pwS2ReconfigVerify()) {
    tmcOk = true;
    Serial.println(F("# driver answered: configuration applied and verified; steering available"));
    return;
  }
  tmcOk = false;
  if (lastLogMs == 0 || nowMs - lastLogMs >= 60000) {
    lastLogMs = nowMs;
    Serial.println(F("# driver not answering (24 V off?); steering waits for it"));
  }
}

// F3 (2026-09-23): a flash write stalls both cores ~20-30 ms, beyond the
// encoder's 20 ms blind-gap limit.  Friction fits are therefore persisted only
// after the wheel has rested 1 s in IDLE or in the landing hold, one write per
// loop pass.
void serviceDeferredSaves(uint32_t nowMs) {
  static uint32_t stillSinceMs = 0;
  if (!frictionSavePendingCw && !frictionSavePendingCcw) { stillSinceMs = 0; return; }
  const bool atRest = preferencesAvailable &&
      (state == ST_IDLE_STOPPED || state == ST_SOFT_HOLD) &&
      encoderMotionReady() && fabsf(omega) <= STILL_REV_S &&
      !(stepper && stepper->isRunning());
  if (!atRest) { stillSinceMs = 0; return; }
  if (stillSinceMs == 0) { stillSinceMs = nowMs; return; }
  if (nowMs - stillSinceMs < 1000) return;
  if (frictionSavePendingCw) {
    frictionSavePendingCw = false;
    if (!pwSaveFrictionDirection(preferences, true, {cw_c, cw_b, cwFitCount}))
      Serial.println(F("# WARN: cw friction persistence failed"));
  } else {
    frictionSavePendingCcw = false;
  ...
  stillSinceMs = 0;  // the write itself blinded the encoder; re-qualify rest
}

// ---- hand-stop detector (see the HAND_* constants) ------------------------
void handReset() {
  handHead = handCount = 0;
  handLastSampleMs = 0;
  handSuspect = false;
}
...
// Sampled every 25 ms on valid encoder data while a RELEASED spin coasts with
// the motor OFF.  Review of flash 2: with the motor energized, a pole slip makes
// the rotor (the encoder is on the motor shaft) swing on the coupling spider,
// and replaying this detector over the recorded 1 kHz traces raised suspects in
// 3 of 13 powered spins with nobody touching the wheel.  So a hand stop during
// braking is NOT alarmed (stage 1a handles it: freeze, verdict, recovery); only
// the motor-off phase can make us sure.  A suspect needs the wheel to come to
// rest soon after; landingVerdict / the free-coast rest decide with
// handStopConfirmed().
void serviceHandDetector(uint32_t nowMs) {
  const bool live = spinOpen && spin.releaseMs != 0 &&
      (state == ST_SPIN_RELEASED || state == ST_TARGET_RESERVED ||
       state == ST_CAPTURE_ARMING) &&
      digitalRead(PIN_EN) == HIGH;
  if (!live || !encoderMotionReady()) return;
  if (handLastSampleMs != 0 && nowMs - handLastSampleMs < 25) return;
  if (handLastSampleMs != 0 && nowMs - handLastSampleMs > 60) handHead = handCount = 0;  // no span across a gap
  handLastSampleMs = nowMs;
  handRing[handHead].ms = nowMs;
  handRing[handHead].v = fabsf(omega);
  handHead = (handHead + 1) % HAND_RING;
  if (handCount < HAND_RING) ++handCount;
  if (handCount < 5) return;
  // newest-first access: idx(0) is the sample just stored
  #define HAND_AT(i) handRing[(handHead + HAND_RING - 1 - (i)) % HAND_RING]
  const float vNow = median3(HAND_AT(0).v, HAND_AT(1).v, HAND_AT(2).v);
  if (handSuspect) {
    if (vNow > 0.6f * handSuspectVmax) handSuspect = false;  // pushed again: not a stop
    return;
  }
  float vMax = 0.0f;
  uint32_t tMax = nowMs;
  for (uint8_t i = 0; i + 2 < handCount; ++i) {
    if (nowMs - HAND_AT(i + 2).ms > HAND_WINDOW_MS) break;
    const float m = median3(HAND_AT(i).v, HAND_AT(i + 1).v, HAND_AT(i + 2).v);
    if (m > vMax) { vMax = m; tMax = HAND_AT(i + 1).ms; }
  }
  #undef HAND_AT
  const uint32_t span = nowMs - tMax;
  if (vMax < HAND_MIN_SPEED_REV_S || span < HAND_MIN_SPAN_MS || vNow > HAND_DROP_FRACTION * vMax)
    return;
  const float decel = (vMax - vNow) * 1000.0f / (float)span;
  if (decel < HAND_DECEL_REV_S2) return;
  handSuspect = true;
  handSuspectMs = nowMs;
  handSuspectVmax = vMax;
  Serial.printf("SPIN#%lu HAND? %.3f -> %.3f rev/s in %lu ms = %.2f rev/s2 (motor+friction max ~0.4); "
                "alarm if it now stops\n",
                (unsigned long)spin.number, vMax, vNow, (unsigned long)span, decel);
}

// At rest: a hand stop if a suspect was seen and the rest began within
// HAND_REST_WITHIN_MS of it (and no re-push cleared it).
bool handStopConfirmed(uint32_t restStartMs) {
  return handSuspect && restStartMs != 0 &&
         (int32_t)(restStartMs - handSuspectMs) >= -200 &&
         (int32_t)(restStartMs - handSuspectMs) <= (int32_t)HAND_REST_WITHIN_MS;
}

void handStopAlarm(const char* where) {
  Serial.printf("SPIN#%lu HAND-STOP (%s): alarm + red strobe; wheel stays where the hand left it (angle %.1f wedge %d)\n",
                (unsigned long)spin.number, where, wheelAngleDeg(), currentWedge());
  handSuspect = false;
  handAlarmPending = true;
}
...
void loop() {
  ...
  serviceDriverReady(nowMs);
  serviceDeferredSaves(nowMs);
  serviceHandDetector(nowMs);
  ...
  switch (state) {
    ...
    case ST_SPIN_RELEASED: {
      ...
      if (speed <= STILL_REV_S) {
        if (settleStillSinceMs == 0) settleStillSinceMs = nowMs;
          ...
          // Owner decision 2 (2026-09-23): stopped by hand -> alarm, no recovery.
          const bool handStop = handStopConfirmed(settleStillSinceMs);
          SpinResult res = handStop ? RES_GUEST_STOPPED
                          : !controlAvailable() ? RES_CONTROL_LOCKED
                          : (spin.hadContact ? RES_GUEST_STOPPED
                                             : RES_NO_REACHABLE_SAFE);
          if (handStop) handStopAlarm("free coast");
          if (!handStop && controlAvailable() && takeoverEnabled &&
    ...
    case ST_LANDING_SETTLE: {
      static uint32_t settleBlindSinceMs = 0;
      static uint32_t settleDragSpin = 0;
      static uint32_t settleRespinSinceMs = 0;
      controlDriverSafe(nowMs);
      setCurrentStage(CS_BRAKE);  // retain CUR_BRAKE_MA through verified settling
      // Pulse train has tapered to zero; the wheel settles under braking
      // current.  Landing requires BOTH interior position AND stillness.
      if (!encoderPositionFresh()) {
        // P1 (2026-09-23): keep the wheel held and wait for the encoder (the
        // verdict needs a fresh position).  After 3 s under a static field the
        // wheel is at rest; only then release it and latch ENCODER_STALE, which
        // clears itself once the encoder is back.
        controlAnomaly(FC_ENCODER_STALE, "stale during settle: holding");
        settleStillSinceMs = 0;
        if (settleBlindSinceMs == 0 || (int32_t)(settleBlindSinceMs - stateEnteredMs) < 0)
          settleBlindSinceMs = nowMs;  // never inherit an older episode
        else if (nowMs - settleBlindSinceMs >= 3000) {
          settleBlindSinceMs = 0;
          closeSpin(RES_FAULTED);
          latchFault(FC_ENCODER_STALE, "encoder lost during settle; resting wheel released after 3 s hold");
        }
        break;
      }
      settleBlindSinceMs = 0;
      trackSyncError();  // flash 3: slips while settling show in SLIP lines / syncErr too
      if (encoderMotionReady() &&
      ...
      // flash 3 (review SHOULD-FIX 1): a gentler re-spin (0.3-0.8 rev/s) is handed
      // back too, once it has lasted REPUSH_CONFIRM_MS without a dip; detection then
      // captures and steers it like any spin.
      if (encoderMotionReady() && fabsf(omega) > SETTLE_RESPIN_REV_S) {
        if (settleRespinSinceMs == 0 || (int32_t)(settleRespinSinceMs - stateEnteredMs) < 0)
          settleRespinSinceMs = nowMs;   // never inherit an older episode
        else if (nowMs - settleRespinSinceMs >= REPUSH_CONFIRM_MS) {
          settleRespinSinceMs = 0;
          Serial.printf("SPIN#%lu RE-PUSH in settle: wheel %.3f rev/s; released as a new spin\n",
                        (unsigned long)spin.number, omega);
          driverFreewheel();
          closeSpin(RES_GUEST_RESPUN);
          startSpinEvent((omega >= 0.0f) ? 1 : -1, nowMs);
          state = ST_SPIN_PUSH;
      ...
      // belongs to the landing verdict, not to guest blame.  Travel well over
      // a wedge means a hand or a slipping rotor.
      // 2026-09-23 flash 3 (P1): never release a moving wheel here.  On 9/23
      // spin 70 slid 55 deg during settle with nobody touching it, and the old
      // release (GUEST_STOPPED, freewheel) let it coast on uncontrolled.  The
      // field now stays where it is: a slipping wheel ratchets to a stop, a
      // hand always wins (~4 N at the rim), and the rest position is judged as
      // usual below (hold, or dare recovery).
      if (fabsf(degreesForCounts(encoderCountsMT - settleEntryCounts)) >
              LANDING_DRAG_ABORT_DEG &&
          encoderMotionReady() && fabsf(omega) > STILL_REV_S &&
          settleDragSpin != spin.number) {
        settleDragSpin = spin.number;
        Serial.printf("SPIN#%lu SETTLE-DRAG: wheel moved %.1f deg during settle; motor keeps holding\n",
                      (unsigned long)spin.number,
                      fabsf(degreesForCounts(encoderCountsMT - settleEntryCounts)));
        controlAnomaly(FC_MOTOR_FIGHT, "wheel moved during settle: motor keeps holding");
      }
      if (encoderMotionReady() && fabsf(omega) <= STILL_REV_S) {
      ...
      if (state == ST_LANDING_SETTLE &&
        ...
        if (encoderMotionReady() && fabsf(omega) > 0.05f) {
          // flash 3 (review MINOR 3): still moving faster than a dare recovery can
          // start (e.g. a slow drag by hand): keep holding and judge it once slower,
          // instead of a verdict that can only give up.
          controlAnomaly(FC_TAKEOVER_TIMEOUT, "settle still moving after 20 s: holding");
          stateEnteredMs = nowMs;
        } else {
          // P1 (2026-09-23): a wheel that never quite stops is still held and is
          // judged where it is (hold, or dare recovery) instead of released.
          controlAnomaly(FC_TAKEOVER_TIMEOUT, "settle never became still: verdict now");
          landingVerdict();
        }
      }
      break;
    ...
    case ST_SOFT_HOLD: {
      ...
      if (stepper && stepper->isRampGeneratorActive()) {
        stepper->forceStop();
        Serial.println(F("# HOLD: pulse generator was running; force-stopped"));
      }
      // Encoder lost while holding (2026-09-23): the wheel rests under a static
      // field, but a push can no longer be seen and a guest would fight the
      // hold.  After 3 s release the resting wheel and latch ENCODER_STALE
      // (LEDs amber); it clears itself once the encoder is back.
      if (!encoderPositionFresh()) {
        if (holdBlindSinceMs == 0 || (int32_t)(holdBlindSinceMs - stateEnteredMs) < 0)
          holdBlindSinceMs = nowMs;  // never inherit an older episode
        else if (nowMs - holdBlindSinceMs >= 3000) {
          holdBlindSinceMs = 0;
          holdAnchorValid = false; holdDeflectSinceMs = 0;
          coastHoldPending = false;   // flash 6
    ...
    case ST_RECOVERY_NUDGE: {
      controlDriverSafe(nowMs);
      // P1 (2026-09-23): the nudge is an open-loop move; a stale encoder lets
      // it finish energized and the verdict waits for fresh feedback.
      const bool fresh = encoderPositionFresh();
      if (!fresh) controlAnomaly(FC_ENCODER_STALE, "stale during recovery: move continues");
      bool still = fresh && !stepper->isRunning() && encoderMotionReady() &&
      ...
      if (nowMs - stateEnteredMs > RECOVERY_TIMEOUT_MS) {
        if (stepper->isRampGeneratorActive()) stepper->forceStop();   // guarded: see landingVerdict
        // P7 (2026-09-23): not a latch any more.  Settle where it is and judge
        // again; the verdict retries while attempts remain, else holds.
        controlAnomaly(FC_LANDING_UNSAFE, "dare recovery timed out: settling for a new verdict");
        setCurrentStage(CS_BRAKE);
        settleStillSinceMs = 0;
        settleEntryCounts = encoderCountsMT;
        state = ST_LANDING_SETTLE;
      ...
      if (recoveryStillSinceMs != 0 && nowMs - recoveryStillSinceMs >= SETTLE_MS) {
        ...
        setCurrentStage(CS_BRAKE);
        state = ST_LANDING_SETTLE;   // verdict re-runs: safe -> hold, dare -> next attempt (max 4), then hold
    ...
    case ST_FAULT_LATCHED:
      ...
      // Auto-clear (2026-09-23, FAILURE_MODES A1): on 9/22 a latched fault left
      // 38 spins unsteered until someone typed r.  Every fault except a failed
      // direction probe, a firmware/STEP-timer fault or an unknown code clears
      // itself once the wheel has rested 3 s with valid encoder feedback and no
      // open spin record.  The driver needs no check here: steering also
      // requires tmcOk, which serviceDriverReady() maintains at rest.
      {
        static uint32_t autoRestSinceMs = 0;
        static uint32_t autoClearCount = 0;
        const bool clearable = faultCode != FC_DIR_CAL_INVALID &&
                               faultCode != FC_STEPPER_API &&
                               faultCode != FC_UNKNOWN_PERSISTED;
        bool resting = encoderMotionReady() && fabsf(omega) <= STILL_REV_S &&
                       !(stepper && stepper->isRunning()) && !spinOpen;
        if (!clearable || !resting) autoRestSinceMs = 0;
        else if (autoRestSinceMs == 0) autoRestSinceMs = nowMs;
        else if (nowMs - autoRestSinceMs >= 3000) {
          autoRestSinceMs = 0;
          Serial.printf("# auto-clear #%lu: %s after 3 s rest; steering %s\n",
                        (unsigned long)++autoClearCount, faultName(faultCode),
                        tmcOk ? "available" : "waits for the driver");
          driverFreewheel();
          faultCode = FC_NONE;
          state = ST_IDLE_STOPPED;
          stateEnteredMs = nowMs;
