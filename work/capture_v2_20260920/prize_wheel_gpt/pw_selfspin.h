...
static void recoverKnownAbort() {
  ...
  // This is available ONLY in a compile-disabled build and never rearms it.
  ...
  if (PW_SELFSPIN_MOTION_ENABLE || active || state != ST_FAULT_LATCHED ||
    ...
    Serial.println(F("# RECOVERY refused: disabled build, exact saved abort, stopped/healthy state and preserved trace required"));
    return;
  }
  portENTER_CRITICAL(&mux);
  inhibit = true;
  ...
  phase = DONE;
  takeoverEnabled = false;
  if (!driverHealthy() || !checkTmcUartRaw() || !pwS2ReconfigVerify() ||
      !driverHealthy() || !encoderMotionReady() || fabsf(omega) > STILL_REV_S ||
      digitalRead(PIN_EN) != HIGH || stepper->isRunning()) {
    Serial.println(F("# RECOVERY refused: health/config recheck; original fault retained"));
    return;
  }
  if (preferences.putUChar(PW_S1_NVS_KEY, 0) != 1 ||
      preferences.getUChar(PW_S1_NVS_KEY, 255) != 0) {
    Serial.println(F("# RECOVERY refused: NVS clear/readback failed; outputs stay inhibited"));
    return;
  }
  persistedFaultRaw = 0;
  faultCode = FC_NONE;
  state = ST_IDLE_STOPPED;
  stateEnteredMs = millis();
  Serial.println(F("# RECOVERY known SELFSPIN_ABORT cleared; EN=1 inhibit=1 consumed=1; no same-boot rearm"));
}
...
bool pwSelfspinCommand(char c) {
  ...
  if (c == 'r') { recoverKnownAbort(); return true; }
  if (active && c != 's' && c != '\r' && c != '\n') {
...
void pwSelfspinPrintStatus() {
  ...
  Serial.printf("# selfspin motion_compiled=%d timer_ready=%d consumed=%d active=%d inhibit=%d "
      ...
      captureAttempted, captureEnergized);
}
