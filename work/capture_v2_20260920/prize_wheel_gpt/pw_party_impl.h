...
void pwPartyBegin() {
  ...
  if (stored != 0 && state != ST_FAULT_LATCHED) {
    persistedFaultRaw = stored;
    faultCode = pwUnknownPersistedFault(stored, (uint8_t)FC_SELFSPIN_ABORT)
        ? FC_UNKNOWN_PERSISTED : (FaultCode)stored;
    state = ST_FAULT_LATCHED;
    stateEnteredMs = millis();
    Serial.printf("# S1: latched fault %s raw=%u restored; takeover locked\n",
                  faultName(faultCode), stored);
  }
#endif
