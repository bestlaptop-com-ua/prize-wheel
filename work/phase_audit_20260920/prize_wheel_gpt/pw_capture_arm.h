...
struct PwCaptureArm {
  ...
  bool begin(uint32_t nowUs, int32_t position, uint32_t hz, bool dirHigh) {
    *this = PwCaptureArm();
    ...
    started = hz >= 40 && hz <= 640; // diagnostic .20 rps ceiling, never > spin-up
  ...
  Verdict update(uint32_t nowUs, int32_t position, bool enHigh, bool dirHigh,
    ...
    if (!motionReady || !isfinite(forwardRevS) || forwardRevS < 0.02f ||
        forwardRevS > 0.30f) return reject(ENCODER);
    const int64_t total = int64_t(position) - int64_t(startPosition);
    if (position < previousPosition || total < 0 || total > 128)
...
// Persisted fault IDs are opaque beyond the last ID this build understands.
inline bool pwKnownSelfspinFault(uint8_t saved, uint8_t current, uint8_t selfspin) {
  return saved == selfspin && current == selfspin && selfspin != 0;
}
inline bool pwUnknownPersistedFault(uint8_t saved, uint8_t lastKnown) {
  return saved != 0 && saved > lastKnown;
}
inline uint8_t pwRestoreFaultWithRecoveryGuard(uint8_t saved, uint8_t guard, uint8_t selfspin) {
  if (pwUnknownPersistedFault(saved, selfspin) || guard == 0) return saved;
  if (guard != selfspin) return 255;
  return saved != 0 ? saved : selfspin;
}
