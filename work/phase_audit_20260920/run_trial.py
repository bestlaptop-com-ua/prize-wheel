...
def readiness(status: dict[str, dict[str, str]], *, armed: bool,
    ...
    try:
        ...
        expected = {"timer_ready": 1, "consumed": 0, "active": 0, "inhibit": 1,
                    ...
                    "recovery_revision": 2, "recovery_pending": 0, "recovery_used": 0}
        if require_enabled:
...
def main() -> int:
    ...
    try:
        ...
        if session is not None and session.camera is not None:
            session.camera.release_requirement()
            try:
                result["camera"] = session.camera.wait_final()
                camera_result = result["camera"]
                if (camera_result.get("process_exit") != 0 or
                        camera_result.get("metadata", {}).get("status") != "captured"):
                    result["camera_evidence_incomplete"] = True
                    exit_code = 1
            except BaseException as exc:
                result["camera_finalize_error"] = type(exc).__name__
                exit_code = 1
            finally:
