...
class CameraSession:
    ...
    def wait_final(self, timeout=60):
        # Serial shutdown/dump handling has already finished. Preserve the
        # finite recording and obtain its final hash before packaging evidence.
        self.record.event("camera_finalizing", timeout_s=timeout)
        try:
            self.process.wait(timeout=timeout)
        except subprocess.TimeoutExpired:
            result = self.result_status()
            result["finalization_error"] = "Recorder did not finish within its finalization deadline"
            return result
