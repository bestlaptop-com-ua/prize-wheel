...
const uint16_t FIGHT_CONFIRM_MS       = 250;  // v2: a bit more margin while the field catches a fast real transient
...
const uint32_t FAS_TRACK_ACCEL_MAX_SPS2 = 16000; // v2: was 6400 (=2.0 rev/s2 ceiling); real wheel transients on the 36in/NEMA23 build reach/exceed that ceiling, so the field cannot track down fast enough and FC_MOTOR_FIGHT trips on lag, not a bad plan
...
const uint16_t CUR_PRECHARGE_MA = 100;  // v2: low enough to be negligible holding torque against a spinning wheel (350 was proven to stall this motor - see DIR_PROBE_CURRENT_MA)
...
void recordDiagnostic(uint32_t dtGoodUs, int16_t delta, uint8_t flags) {
  if (!diagnosticCapture || diagnosticFrozen) return;
...
void setCurrentStage(CurrentStage next) {
  ...
  switch (next) {
    ...
    case CS_PRECHARGE:
      ...
      break;
    case CS_CAPTURE: {
      // v2: ramp instead of step - an instantaneous current jump at this
      // transition was exciting a real torsional ring in the drivetrain
      // (encoder-confirmed on hardware, 2026-09-18: raw delta 1->17 then
      // reverses right at the step). 6 sub-steps over ~6 ms is enough to
      // avoid the impulsive excitation without adding meaningful latency.
      uint16_t from = g_currentMa > 0 ? g_currentMa : CUR_PRECHARGE_MA;
      for (uint8_t i = 1; i <= 6; ++i) {
        uint16_t step = from + (uint32_t)(CUR_CAPTURE_MA - from) * i / 6;
        driver.rms_current(step, 1.0);
        delayMicroseconds(1000);
      }
      g_currentMa = CUR_CAPTURE_MA;
      break;
    }
...
void handleCommandChar(char command) {
  switch (command) {
    ...
    case 'x':
      ...
      break;
    case 'g': {
      // Temporary read-only AS5600 AGC/status check - no interaction with the control loop.
      Wire.beginTransmission(0x36); Wire.write(0x0B);
      uint8_t st = 0xFF;
      if (Wire.endTransmission(false) == 0 && Wire.requestFrom((uint8_t)0x36, (uint8_t)1) == 1) st = Wire.read();
      Wire.beginTransmission(0x36); Wire.write(0x1A);
      uint8_t agc = 0xFF;
      if (Wire.endTransmission(false) == 0 && Wire.requestFrom((uint8_t)0x36, (uint8_t)1) == 1) agc = Wire.read();
      Serial.printf("# AS5600 STATUS 0x%02X (MD=%d ML=%d MH=%d)  AGC=%u (want ~40-200)\n", st, (st>>5)&1, (st>>4)&1, (st>>3)&1, agc);
      break;
    }
    case 'r':
