...
data=bytearray()
try:
 assert (p/'gentle6_wheel_build.exit').read_text().strip()=='0'
 assert (p/'speedup_replay_verified.exit').read_text().strip()=='0'
 with (p/'gentle6_wheel_upload.log').open('w') as f:
  r=subprocess.run([cli,'upload','--fqbn',fqbn,'--port','COM7','--input-dir',str(p/'gentle6_wheel-firmware'),str(p.parent/'prize_wheel_gpt')],stdout=f,stderr=subprocess.STDOUT,timeout=90)
 (p/'gentle6_wheel_upload.exit').write_text(str(r.returncode))
 ...
 try:
  ...
  boot=read_for(5)
  assert 'v2-gentle6-20260918' in boot,'wrong build'
  ...
  (p/'gentle6_wheel_verified.exit').write_text('0')
  binary=p/'gentle6_wheel-firmware/prize_wheel_gpt.ino.bin'
  (p/'gentle6_wheel.sha256').write_text(hashlib.sha256(binary.read_bytes()).hexdigest())
 finally:
  (p/'gentle6_wheel_verified_boot.log').write_bytes(data)
  if port.is_open:port.close()
except Exception:(p/'gentle6_wheel_final_upload.error').write_text(traceback.format_exc())
