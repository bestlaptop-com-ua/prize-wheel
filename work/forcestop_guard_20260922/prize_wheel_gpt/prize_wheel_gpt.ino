...
void landingVerdict() {
  ...
  if (stepper && stepper->isRunning()) stepper->forceStop();   // 2026-09-22: guarded - on an idle stepper forceStop() leaves the FastAccelStepper one-shot stop flag armed and it cancels the next capture (AUDIT_2026-09-22.md s1)
...
/* ========================================================================== */
void help() {
  Serial.println(F("# build: imbalance-model-20260921 PARTY + forcestop-guard-20260922; based on plywood-capture-review-20260920; takeover defaults OFF"));
  Serial.println(F(
  ...
  switch (state) {
    ...
    case ST_RECOVERY_NUDGE: {
      ...
      if (nowMs - stateEnteredMs > RECOVERY_TIMEOUT_MS) {
        if (stepper->isRunning()) stepper->forceStop();   // guarded: see landingVerdict
