...
uint16_t rawZero = 3807;  // label-true anchor: AS5600 raw at leading edge of wedge 0 (owner-measured 2026-07-28); 'z' re-anchors and persists
static inline int32_t staticCountsFromRaw(uint16_t raw) { int32_t sc = ((int32_t)rawZero - (int32_t)raw) % 4096; if (sc < 0) sc += 4096; return sc; }
...
void primeEncoder(uint16_t raw, uint32_t doneUs, bool preserveNearestTurn) {
  if (!encoderPrimed || !preserveNearestTurn) {
    encoderCountsMT = staticCountsFromRaw(raw);  // label-true seed (frame fix 2026-07-28)
  } else {
    int32_t wholeTurnBase = encoderCountsMT - (encoderCountsMT % 4096);
    int32_t candidate = wholeTurnBase + staticCountsFromRaw(raw);  // label-true snap
...
float wheelAngleDeg() {
  double angle = fmod(angleDegMT, 360.0);  // label-true by construction (rawZero-seeded frame)
  ...
  switch (command) {
    case 'z':
      if (!encoderPrimed) Serial.println(F("# encoder is not primed; calibration ignored"));
        ...
        rawZero = lastGoodRaw; preferences.putUShort("rawZero", rawZero);
        primeEncoder(lastGoodRaw, micros(), false);
        Serial.printf("# raw-zero anchor set: raw=%u (persisted; park wedge-0 leading edge first)\n", rawZero);
      }
      break;
    case 'p':
      startDirectionProbe();
